...
export const TRADE_BOOT_REQUEST = `Return ONE Apply JSON object only — copyable from { through }. No explanation, Prompt IDs, Markdown fences, or comments in the paste payload.
...
function formatProspectsRoster(prospects: TradeProspect[]): string {
  ...
  lines.push("");
  lines.push("Paste trade-proposal in Control → Apply (Scout war room has boot package).");
  return lines.join("\n");
}
