...
const SCOPED_AI_REQUEST = `Return ONE Apply JSON object only — copyable from { through }. No explanation, Prompt IDs, Markdown fences, or comments in the paste payload.
...
- decision-update: planId, verdict (go|wait|probe|no), decisionConfidence, challenges[] required; optional thesisQuality, opportunityQuality, confirmationCost (supplied prices only), locationEvidence, confirmationEvidence, singleEntryOnly, reasoning, planningRisk{}, executionRisk{}, probe{} when verdict=probe, layeredEntry{executionMethod,limits[{price,allocationPercent}]} when verdict=go; executionInstruction REQUIRED when geometry (plannedEntry|stopPrice|targetPrice|layeredEntry) is present — write share counts in the sentence; never plannedQuantity; see md/matrix/execution-instruction-spec.md
- layered-entry-update: planId, filledThroughIndex (integer >= -1, 0-based, -1=none) or status (planned|partial|full|missed|active|cancelled) — fill/lifecycle only; never limits/risk/sizing
