...
export function buildMatrixMechanicsBrief(
  ...
  return [
    ...
    "North Star: learn from every decision to improve future decisions, increase positive expectancy, and ultimately generate sustainable positive returns.",
    ...
    "PLAN EVALUATION DISCIPLINE, EXECUTABLE SWING PLAN (one-pass), Analysis/Apply discipline,",
    ...
    "Apply schema contract lives under Control → Apply — only for explicit Apply Mode.",
    "",
    ...
    "- Closed trades are ledger history — do not recreate the ticker or reuse a closed H00x id to \"replace\" them.",
    "- Controlled corrections on closed trades use trade-update (legacy completion / datesReconstructed with audit) — never invent real Playbook/PLAN links.",
    ...
    "Learning Outcome (LO-xxx): executed_win|executed_loss|missed_opportunity|cancelled|expired|unexecuted_plan_loss — auto on trade close / plan outcome.",
    ...
    "",
    "CASE D — EXECUTION / PLAN DIVERGENCE (MXT 028)",
    "Use Case D when actual execution differs materially from an evaluable planned path (no fill or deficient execution).",
    "Preserve realized P/L/R separately from planned/counterfactual outcome.",
    "Counterfactual R must be symmetric: missed upside = +R CF; avoided planned loss = −R CF.",
    "Counterfactual R must NEVER be added to actual portfolio P/L; label it as counterfactual/planned R in Matrix / Insights.",
    "Subtypes: D1 No Entry/Would Profit · D2 No Entry/Would Loss · D3 No Entry/Indeterminate · D4 Deficient Exec/Would Profit · D5 Deficient Exec/Would Loss · D6 Deficient Exec/Indeterminate.",
    "Case D classification does NOT determine MAF component attribution (thesis/zone/entry/stop/execution/management/timing/capital remain independent).",
    "T0 freeze binding is Plan-specific: never inherit another Plan's T0 merely because stockThesisId is shared.",
    "",
    "PLAN EVALUATION DISCIPLINE",
    "A Plan is a falsifiable tactical model of an opportunity, not merely a defensible narrative.",
    "When Reality is available, evaluate whether the market behaved approximately within the structure the Plan expected.",
    "Reality is used to FALSIFY the Plan, not to rationalize it.",
    "Thesis quality and Plan quality are independent.",
    "Correct direction does not make the Plan correct.",
    "A correct/reached target does not make the Plan correct.",
    "0R does not imply a neutral, acceptable, or successful Plan.",
    "entryReached=false is insufficient by itself to evaluate Plan quality.",
    "Examine the realized relevant structure against the ORIGINAL Plan geometry.",
    "Entry→Stop distance may be used as a normalized analytical reference for structural displacement.",
    "This analytical distance NEVER changes Realized R or Counterfactual R.",
    "Material structural displacement may establish that the Plan was wrong even when the Thesis was correct.",
    "Do not use hindsight to invent the entry that should have been taken.",
    "Plan failure does not imply permission to chase.",
    "Establishing Plan failure and explaining its cause are separate analytical tasks.",
    "Do not force entry, timing, playbook, ticker-behavior, or other causal attribution merely to complete the record.",
    "If T0 + Reality do not support the cause, use inconclusive.",
    "MXT MUST be able to terminate analysis with: The Plan was wrong; the cause is not known.",
    "Library owner of provisional ~2–3× Entry→Stop investigation band: md/matrix/mxt-plan-failure-evaluation.md.",
    "Mechanics MUST NOT convert that experimental band into a hard threshold, automatic classification, deterministic metric, or proof that Plans inside the band are correct.",
    "SHOP · PLAN-005 anti-misinterpretation example:",
    "  Original: Entry 136 · Stop 125 · Target 180 · Entry→Stop distance = 11 → inspect 136 / 147(+1) / 158(+2) / 169(+3) / 180(target).",
    "  Reality did not validate ~133–138.75; opportunity developed materially higher and later reached/exceeded 180.",
    "  CORRECT: Thesis broadly correct · PLAN-005 wrong as tactical model · no chase good discipline · Realized R 0R · Counterfactual R 0R under sealed R semantics · cause may remain inconclusive.",
    "  FORBIDDEN: (1) target reached ⇒ Plan good; (2) 136 had historical justification ⇒ Plan approximately correct despite material geometry falsification;",
    "  (3) SHOP went to 180+ ⇒ we should have bought higher (hindsight / chase); (4) later structure resembles another Playbook ⇒ that Playbook was definitely identifiable at T0 without T0 evidence;",
    "  (5) cannot explain cause ⇒ cannot classify Plan as failed. MXT may know WHAT failed without knowing WHY.",
    "",
    "DATA CORRECTABILITY (MXT 032)",
    "Historical truth is protected from hindsight; recorded errors remain repairable.",
    "Apply schema freshness: Control → Apply → Apply schema contract must show schemaVersion 2026-09-16.mxt-15-35-ole-methodology (and acceptedTypes must list layered-entry-update, thesis-t0 and plan-delete). If missing → STALE paste/build.",
    ...
    "Legacy Trade dates: trade-update datesReconstructed + dateCorrectionNote (existing path).",
    "Hindsight price/outcome alone is never sufficient to rewrite a historical decision.",
    "",
    "VOCABULARY DISCIPLINE",
    "Use the smallest stable MXT vocabulary needed to operate the product.",
    "Prefer the exact existing ontology: Plan, T0, Reality, Execution, Outcome, Case, MAF, Improvement, Realized R, Counterfactual R.",
    "Do not introduce new labels, statuses, categories, metrics, prefixes, or synonyms unless MXT lacks a necessary distinction.",
    "If a new term is genuinely required: it must name a distinct concept, have an operational purpose, be defined once concisely, and then be used consistently with no synonyms.",
    "Prefer removing terminology over expanding the vocabulary.",
    "Schema-required technical values must stay minimal and canonical; do not promote transient validator details into human-facing concepts unless MXT explicitly requires that.",
    "Do not expose internal implementation vocabulary in UI merely because it exists in code.",
    "",
    ...
    "Before ANY Apply JSON: open Control → Apply, then copy the visible row labeled Apply schema contract.",
    "Never invent JSON keys, enums, nesting, or formats. Use only schema / accepted example / validator feedback.",
    "If that exact copy row is unavailable: stop. Ask the human to open Control → Apply and copy Apply schema contract — deliver conceptual analysis only.",
    ...
    "OLE under INSUFFICIENT fill evidence: identify defensible zone, distribute authorized risk reasonably across layers, document uncertainty — no universal % default (PLAN-015 exemplar in Apply contract).",
    ...
    "- Use trade-update on an existing Trade id (including legacy closed completion / date repair). Do not invent a Trade via trade-update before one exists — use trade-proposal.",
    "",
    "ASYMMETRY RULE",
    ...
    "Same dollar cost if the trade fails — risk budget (1R $) stays controlled so R multiples are comparable.",
    "Entry Optimization is NOT maximize theoretical R alone: Optimized Entry = Target→Stop→R Map→Participation→entry under fixed R$.",
    "R quality remains a hard gate (minimumRR); participation/no-fill is a co-equal dimension — MAX R ≠ OPTIMIZED ENTRY.",
    ...
    "- Fixed 1R risk budget split by expectancy weight — ONLY when fill/expectancy evidence supports unequal concentration.",
    "- When FILL EVIDENCE: INSUFFICIENT: reasonable risk split across defensible layers (PLAN-015 shows one exemplar) — NOT false-precision concentration without evidence.",
    "- Do not claim statistical optimization for uncertainty-distributed layering; do not fabricate fill probabilities.",
    "- Single common stop (structural invalidation) — NOT independent stops per layer.",
    "- Sizing: shares_i = (r_i × R$) / (E_i − S). Do not auto-consume full risk budget if integer shares make that impossible.",
    ...
    "- layered-entry-update can INITIALIZE or UPDATE layeredEntry on an EXISTING Scout Plan (never duplicate the Plan).",
    "",
    ...
    "  2. Control → Apply → copy row Apply schema contract",
