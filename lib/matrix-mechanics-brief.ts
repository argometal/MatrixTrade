...
export function buildMatrixMechanicsBrief(): string {
  return [
    ...
    "Learning Outcome (LO-xxx): executed_win|executed_loss|missed_opportunity|cancelled|expired|unexecuted_plan_loss|duplicate_creation — trade close / plan-outcome Apply.",
    "plan-outcome Apply: close Scout without Trade (UPL / miss / cancel / expire / duplicate). Server derives counterfactualR; never invent fills. duplicate_creation → excludedFromMetrics.",
