...
export function buildMatrixMechanicsBrief(
  ...
  return [
    ...
    "",
    "DATA CORRECTABILITY (MXT 029)",
    "Historical truth is protected from hindsight; recorded errors remain repairable.",
    "recordKind: original | reconstructed | corrected. Corrections change the effective value; prior values stay in correctionAudit.",
    "Apply schema freshness: Control → Apply → Apply schema contract must show schemaVersion 2026-09-05.mxt-029-correctability (and acceptedTypes must list thesis-t0-repair). If missing → STALE paste/build.",
    "T0: Apply type thesis-t0-repair — reconstructed (Missing T0) or corrected (wrong freeze). Requires note + evidence. Never invent silently. Never share freezes across Plans.",
    "Evaluation/hindsight reconstruction ≠ automatic T0 mutation; explicit audited thesis-t0-repair is the only T0 body repair path.",
