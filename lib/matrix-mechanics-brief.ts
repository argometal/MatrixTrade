...
export function buildMatrixMechanicsBrief(): string {
  return [
    ...
    "Every operational analysis/handoff response must begin with its Prompt ID.",
    "Exception — Apply JSON paste: when the user asked for Apply JSON, the copyable payload is only the JSON object ({ through }). Do not prefix Prompt IDs, block IDs, or commentary onto that object.",
    ...
    "APPLY JSON PASTE: when Apply JSON is requested, deliver one JSON object as the directly copyable payload. The human copies only from { through } into Control → Apply. Never include explanation, introduction, conclusion, Prompt IDs, block IDs, extra Markdown, or comments in that paste. Surrounding text is rejected by the parser — do not wrap the object.",
    "LAYERED ENTRY instruction: AI writes share quantities in executionInstruction (e.g. Buy 1 share at $315. Buy 2 shares at $310. Buy 2 shares at $305.). allocationPercent on limits[] is the structural distribution. Do not send plannedQuantity in Apply JSON — Matrix computes persisted quantities.",
    ...
    "Only then generate exactly one Apply-ready JSON object — copyable from { through }; no wrapping prose.",
    ...
    "Plan Map execution sentence: AI-authored proposal.executionInstruction only (PM-style operational instruction). REQUIRED on scout-plan-create and geometry-mutating decision-update — schema Validate rejects omissions. Spec: md/matrix/execution-instruction-spec.md. Never a deterministic template. Layered: write share counts in the sentence (e.g. Buy 1 share at $315. Buy 2 shares at $310. Buy 2 shares at $305.). allocationPercent stays on limits[] (structural). Do not send plannedQuantity. Omit unknown prices/risk. Explanation layer — does not change Matrix math.",
