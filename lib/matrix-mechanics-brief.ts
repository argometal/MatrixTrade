...
export function buildMatrixMechanicsBrief(): string {
  return [
    ...
    "AUTHORITATIVE EXECUTION PAIR (non-negotiable): every accepted Scout execution preserves (1) structured layeredEntry / plan levels AND (2) a human-readable execution description.",
    "The description is a deterministic projection of structured execution (shares or allocation %, stop, primary target) — never the calculation source, never parsed from reasoning/notes.",
    "Layered analysis must NEVER persist only as prose while the operative plan remains single-entry. decision-update / scout-plan-create concluding layered execution MUST include structured layeredEntry.limits[] (min 2, alloc sum 100%).",
    "Plan Map and Scout snapshots show the same description; changing shares, layers, stop, or target regenerates it.",
    "Reject/flag a layered description when the stored plan has fewer than 2 entry layers.",
