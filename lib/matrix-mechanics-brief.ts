...
export function buildMatrixMechanicsBrief(): string {
  return [
    ...
    "- file-update.initialScout backfills a Scout Plan ONLY when the Stock File has NO linked active plan — it is a repair, not a second stock-case-create.",
    "- When an active plan already exists and the human wants a fresh window, ask for / propose a new PLAN (do not reuse the finished plan as if it were blank).",
    "",
    ...
    "- Incomplete / open fills stay in Scout war room until completed (closed + review).",
    "",
    ...
    "Pipeline: each TF independently → per-TF report → hierarchy integration → technical summary → JSON export only.",
    ...
    "MTAE FORBIDDEN outputs: maximumEntry, recommendedEntry, minimumRR, shares, Scout verdict, position sizing.",
    "Accept: technical-assessment → store assessment + patch Stock File synthesis (levels/invalidation/historicalAnalysis).",
    "Human corrections: technical-calibration (procedure errors) — train Matrix consistency, not P/L outcomes.",
    ...
    "",
    "RULE OF GOLD",
    "Playbook → Stock File (optionally filled via MTAE) → Scouting Desk → Trade.",
    ...
    "New opportunity on the SAME ticker: keep the Stock File → open a NEW Scout Plan window → trade-proposal with a NEW H00x id.",
    ...
    "",
    "SNAPSHOT MENU (ask human to copy when you need depth)",
    "Human copies via Control panel (Mechanics brief / Technical analysis / Playbook / Stock file / Scout desk) or the Trade window snapshot.",
    "- Matrix Mechanics brief — rules primer (Control → Mechanics brief; paste once per chat)",
    "- MTAE protocol — technical analysis procedure (Control → Technical analysis; charts → technical-assessment)",
    "- Playbook snapshot — strategies + closed-trade stats per playbook (Control → Playbook or /playbook)",
    "- Scout desk overview — all stock files + scouts + monthly risk room",
    "- {TICKER} stock snapshot — one profile + evidence + scouts",
    "- {TICKER} PLAN-xxx scout snapshot — single plan: strategy stop, entry, target, decision",
    "- {TICKER} profile snapshot — dossier + invalidation (structural context only)",
    "- {TICKER} linked scouts — active PLANs for that ticker",
    "- Trades snapshot / {ID} trade snapshot — execution records (Trades window)",
    "- {ID} forensic snapshot — closed trade deep export ONLY on /trades/{ID} (never in Control)",
    "Request a specific slice by name instead of guessing missing context.",
    "FORBIDDEN: do not ask the human to open Control → Closed trade / Session / Case — those labels are retired.",
    "",
