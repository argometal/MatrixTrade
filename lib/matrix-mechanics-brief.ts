...
export function buildMatrixMechanicsBrief(): string {
  return [
    ...
    "Plan Map execution sentence: AI-authored proposal.executionInstruction only (PM-style operational instruction). REQUIRED on scout-plan-create and geometry-mutating decision-update — schema Validate rejects omissions. Spec: md/matrix/execution-instruction-spec.md. Never a deterministic template. Never invent prices/shares/risk. Layered: Buy {qty} shares ({allocationPercent}%) at ${price} — Matrix binds {qty} from plannedQuantity; never compute shares. Omit unknowns. Explanation layer — does not change Matrix math.",
