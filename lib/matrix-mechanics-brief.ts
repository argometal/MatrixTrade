...
export function buildMatrixMechanicsBrief(): string {
  return [
    ...
    "Entry Optimization means solving for the highest admissible entry price that still satisfies the required minimum R using the probable target and structural stop. Technical analysis is then used only to determine whether that entry is realistic.",
    ...
    "In Analysis Mode, prioritize finding the entry that maximizes initial R before any Apply block is requested.",
    "",
    ...
    "3. Solve the MAXIMUM ENTRY PRICE.",
    ...
    "5. Only after the mathematical limit is known should technical analysis begin.",
    "   Technical analysis is used to determine whether the required entry is realistically obtainable, not to redefine the mathematics.",
    ...
    "Human copies via Control panel → pick layer → copy snapshot text.",
    "- Matrix Mechanics snapshot — full rules (paste once per session)",
    "- Dashboard snapshot — budget, experiment, attention queue",
    "- Playbook snapshot — strategies + closed-trade stats per playbook",
    ...
    "- Trades snapshot / {ID} trade snapshot — execution records",
    "- {ID} forensic snapshot — closed trade deep export (legacy gaps, R, review, post-stop)",
