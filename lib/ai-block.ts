...
- decision-update: scout decision or tactical correction — planId required; decision mode needs verdict, decisionConfidence, challenges[]; tactical mode needs at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry, executionInstruction; geometry changes REQUIRE executionInstruction (layered: Buy {qty} shares (alloc%) at $price — never compute shares)
...
- scout-plan-create: NEW PLAN on existing Stock File — stockFileId, ticker, plannedEntry, stopPrice, targetPrice, REQUIRED executionInstruction (layered: Buy {qty} shares (alloc%) at $price); optional layeredEntry; optional verdict+challenges; allocates NEW PLAN-xxx (same-ticker new window)
...
const SAMPLE_BLOCKS: Record<AiBlockType, Record<string, unknown>> = {
  ...
  "scout-plan-create": {
    ...
    proposal: {
      ...
      executionInstruction:
        "Buy {qty} shares (30%) at $215. Buy {qty} shares (40%) at $210. Buy {qty} shares (30%) at $205. Use the common stop at $200 for the full position. Hold until the primary target at $260 unless the thesis changes. Do not chase above the starter. Any layer not reached remains unfilled.",
      thesis:
