...
- decision-update: scout decision on PLAN — planId required; decision mode: verdict (go|wait|probe|no), decisionConfidence (0-100), challenges[] (min 1); tactical mode: at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry, familyBAssessment, operationalAssessment (verdict optional); when analysis concludes layered execution MUST include structured layeredEntry{stopModel,sizingMode,authorizedRiskAmount,primaryTargetPrice,commonStopPrice,limits[{price,allocationPercent,role?,stopPrice?,rationale?,confidence?}]} (min 2 limits, alloc=100%) — never leave layers only in reasoning/notes; Matrix projects the broker-actionable execution description from that structure (not from prose); operationalAssessment may include thesisState, operationalState, waitHorizon, nextAction, freshness, currentExecutableRR, reviewRequired, reasonCodes[], nextReviewAt; Matrix recomputes R/risk derived fields server-side — do not forge rr; optional thesisQuality, opportunityQuality (0-100), confirmationCost{...}, locationEvidence, confirmationEvidence, singleEntryOnly, reasoning (preserved separately — not an execution source), planningRisk{}, executionRisk{}, probe{} when verdict=probe
...
- decision-update: scout decision or tactical correction — planId required; decision mode needs verdict, decisionConfidence, challenges[]; tactical mode needs at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry; layered conclusions require structured layeredEntry (not notes-only)
...
export const AI_BLOCK_SAMPLE_OPTIONS: AiBlockSampleOption[] = [
  ...
  },
  {
    ...
    hint: "verdict + confidence + challenges; layered conclusions MUST include layeredEntry{} (structured limits — projection becomes Plan Map description; never notes-only); probe{} when verdict=probe",
  },
  {
