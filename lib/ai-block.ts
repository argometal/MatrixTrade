...
export const SCOUTING_AI_BLOCK_REQUEST = `Return ONE Apply JSON object only — copyable from { through }. No explanation, introduction, conclusion, Prompt IDs, block IDs, Markdown fences, or comments in the paste payload.
Required shape:
{
  "type": "<block-type>",
  "proposal": { ... }
...
- decision-update: scout decision on PLAN — planId required; decision mode: verdict (go|wait|probe|no), decisionConfidence (0-100), challenges[] (min 1); tactical mode: at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry, familyBAssessment, operationalAssessment, executionReadiness, executionInstruction (verdict optional); when plannedEntry|stopPrice|targetPrice|layeredEntry present, executionInstruction is REQUIRED (AI Plan Map sentence — write share counts in prose, e.g. Buy 1 share at $315; never send plannedQuantity; allocationPercent stays on limits[]); layeredEntry may include stopModel, sizingMode, authorizedRiskAmount, primaryTargetPrice, commonStopPrice, limits[{price,allocationPercent,role?,stopPrice?,rationale?,confidence?}]; operationalAssessment may include thesisState, operationalState, waitHorizon, nextAction, freshness, currentExecutableRR, reviewRequired, reasonCodes[], nextReviewAt; Matrix recomputes R/risk derived fields server-side — do not forge rr; optional thesisQuality, opportunityQuality (0-100), confirmationCost{...}, locationEvidence, confirmationEvidence, singleEntryOnly, reasoning, planningRisk{}, executionRisk{}, probe{} when verdict=probe, layeredEntry when verdict=go
- layered-entry-update: record fill/status on EXISTING PLAN layeredEntry — required planId + filledThroughIndex (integer >= -1, 0-based, -1=none) OR status (planned|partial|full|missed|active|cancelled). Fill only — NEVER send limits/authorizedRiskAmount/sizingMode. Configure ladders via scout-plan-create or decision-update.layeredEntry without fill fields.
...
- scout-plan-create: NEW Scout Plan window on an EXISTING Stock File — stockFileId (or stockThesisId), ticker, plannedEntry, stopPrice, targetPrice required; REQUIRED executionInstruction (AI Plan Map sentence — layered: write share counts e.g. Buy 1 share at $315; allocationPercent on limits[]; never plannedQuantity; see md/matrix/execution-instruction-spec.md); optional layeredEntry{stopModel,sizingMode,authorizedRiskAmount,primaryTargetPrice,commonStopPrice,limits[{price,allocationPercent,role?,stopPrice?,rationale?,confidence?}]} (persist structured layers — never leave layers only in notes/reasoning), optional verdict+decisionConfidence+challenges, playbookId/playbookIds, status (watching|ready|active), thesis, notes, reasoning. Server allocates a NEW PLAN-<n> (min 3-digit pad; PLAN-1000+ allowed). Do NOT supply id/planId. Do NOT use stock-case-create for same ticker. Do NOT reuse an old planId.
...
- Return exactly one JSON object. No arrays of blocks. Paste payload is { through } only.
...
You infer the correct internal block type and return exactly ONE Apply JSON object — copyable from { through }. No explanation, Prompt IDs, block IDs, Markdown fences, or comments in the paste payload.
...
- decision-update: scout decision or tactical correction — planId required; decision mode needs verdict, decisionConfidence, challenges[]; tactical mode needs at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry, executionInstruction; geometry changes REQUIRE executionInstruction (layered: write share counts in the sentence; never plannedQuantity)
- layered-entry-update: record fill/status on existing PLAN — planId + filledThroughIndex (integer >= -1, 0-based, -1=none) OR status (planned|partial|full|missed|active|cancelled); never limits/risk
...
- scout-plan-create: NEW PLAN on existing Stock File — stockFileId, ticker, plannedEntry, stopPrice, targetPrice, REQUIRED executionInstruction (layered: Buy 1 share at $315 …; allocationPercent on limits[]; never plannedQuantity); optional layeredEntry; optional verdict+challenges; allocates NEW PLAN-xxx (same-ticker new window)
...
export const AI_BLOCK_SAMPLE_OPTIONS: AiBlockSampleOption[] = [
  ...
  },
  {
    ...
    hint: "planId + filledThroughIndex (integer >= -1, 0-based, -1=none) OR status (planned|partial|full|missed|active|cancelled) — fill/lifecycle only; never limits/risk",
  },
  {
...
const SAMPLE_BLOCKS: Record<AiBlockType, Record<string, unknown>> = {
  ...
  "decision-update": {
    ...
    proposal: {
      ...
      executionInstruction:
        "Do not execute yet. Wait for a pullback into 340-355. If reached with R:R still >= 3 versus the planned stop below 320, reassess for entry — do not chase above the zone.",
  ...
  "scout-plan-create": {
    ...
    proposal: {
      ...
      executionInstruction:
        "Buy 1 share at $215. Buy 2 shares at $210. Buy 2 shares at $205. Use the common stop at $200 for the full position. Hold until the primary target at $260 unless the thesis changes. Do not chase above the starter. Any layer not reached remains unfilled.",
      thesis:
