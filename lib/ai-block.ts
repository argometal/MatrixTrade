...
- plan-outcome: terminal Scout plan without Trade — planId + outcomeKind (unexecuted_plan_loss|duplicate_creation|expired_window). UPL requires entryReached, stopReachedBeforeTarget, targetReachedBeforeStop, nonExecutionReason; server derives realizedR=0, counterfactualR=-1. expired_window requires only planId (notes optional) — closes PLAN window only; LO kind=expired; no counterfactual loss; Stock File may stay active. Never invent fills/risk; do not use decision-update or fictitious Trade
