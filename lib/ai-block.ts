...
- decision-update: scout decision on PLAN — planId required; decision mode: verdict (go|wait|probe|no), decisionConfidence (0-100), challenges[] (min 1); tactical mode: at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry, familyBAssessment, operationalAssessment, executionReadiness, executionInstruction (verdict optional); layeredEntry may include stopModel, sizingMode, authorizedRiskAmount, primaryTargetPrice, commonStopPrice, limits[{price,allocationPercent,role?,stopPrice?,rationale?,confidence?}]; operationalAssessment may include thesisState, operationalState, waitHorizon, nextAction, freshness, currentExecutableRR, reviewRequired, reasonCodes[], nextReviewAt; executionInstruction = AI Plan Map operational sentence (PM tone; never invent prices/shares/risk; omit unknowns); Matrix recomputes R/risk derived fields server-side — do not forge rr; optional thesisQuality, opportunityQuality (0-100), confirmationCost{...}, locationEvidence, confirmationEvidence, singleEntryOnly, reasoning, planningRisk{}, executionRisk{}, probe{} when verdict=probe, layeredEntry when verdict=go
...
- decision-update: scout decision or tactical correction — planId required; decision mode needs verdict, decisionConfidence, challenges[]; tactical mode needs at least one of plannedEntry, stopPrice, targetPrice, minimumRR, thesis, notes, validUntil, status, layeredEntry, executionInstruction
...
- scout-plan-create: NEW PLAN on existing Stock File — stockFileId, ticker, plannedEntry, stopPrice, targetPrice; optional layeredEntry (structured limits — required when analysis concludes layered execution); optional executionInstruction (AI Plan Map sentence); optional verdict+challenges; allocates NEW PLAN-xxx (same-ticker new window)
...
const SAMPLE_BLOCKS: Record<AiBlockType, Record<string, unknown>> = {
  ...
  "scout-plan-create": {
    ...
    proposal: {
      ...
      executionInstruction:
        "Buy the first 30% at $215. Add 40% at $210 if reached and complete the position at $205. Use the common stop at $200 for the full position. Hold until the primary target at $260 unless the thesis changes. Do not chase above the starter. Any layer not reached remains unfilled.",
      thesis:
