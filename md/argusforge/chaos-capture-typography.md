...
## What we had
- `window.prompt` for text body → tiny, unusable for long fragment capture
- Content editor: mono `text-sm` + markdown structure toolbar (H1/H2…) — fights “ingesta”

## Target UX
Word-like capture window for a **Fragment**: wide (`max-w-3xl`), full-height textarea, optional title, no decorative headings.

Naming: binder = **Chaos Deck**; unit = **Fragment** — see `chaos-deck-fragment-naming.md`.
...
| Measure | **Wide for capture** | Reading optima often cite ~50–75ch; capture prioritizes writing space over editorial measure |
...
- Classification during capture

## Implementation
`ChaosCaptureSurface` — full-viewport dialog from Chaos Deck “Add fragment (text)”.
