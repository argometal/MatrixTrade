...
| `evaluate_expired_plan` | Plan failed/expired/skipped + !outcome.recordedAt | Plan, Stock File | `/planning?plan=` | `plan-outcome` (also Planning UI `recordPlanOutcomeAction`) | outcome.recordedAt (then LO/OBS/MAF as derived tasks) | Yes | **SUPPORTED** |
...
`evaluate_expired_plan` is **SUPPORTED** via Apply type `plan-outcome` (kinds `unexecuted_plan_loss` / `duplicate_creation` / standard outcome record). Planning UI `recordPlanOutcomeAction` remains available.
