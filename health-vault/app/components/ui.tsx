...
export function Button({
  children, href, onClick, variant = "primary", type = "button", fullWidth = false, className = "",
}: {
  children: React.ReactNode; href?: string; onClick?: () => void;
  variant?: "primary" | "secondary" | "ghost" | "danger"; type?: "button" | "submit";
  fullWidth?: boolean; className?: string;
}) {
  const base = "inline-flex items-center justify-center rounded-xl px-5 py-3.5 text-base font-semibold transition active:scale-[0.98]";
  const variants = {
...
export function PageHeader({ title, subtitle, backHref }: { title: string; subtitle?: string; backHref?: string }) {
  return (
    <div className="mb-6">
      {backHref && <Link href={backHref} className="mb-2 inline-block text-sm text-zinc-400 hover:text-zinc-200">← Volver</Link>}
...
export function StatCard({ label, value, accent }: { label: string; value: string | number; accent?: "teal" | "red" | "amber" }) {
  const colors = { teal: "text-teal-400", red: "text-red-400", amber: "text-amber-400" };
  return (
    <Card>
...
export function EmptyState({ message }: { message: string }) {
  return <Card className="py-8 text-center"><p className="text-zinc-400">{message}</p></Card>;
}

export function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="space-y-1.5"><label className="block text-sm font-medium text-zinc-300">{label}</label>{children}</div>;
}
...
export function TypeBadge({ type }: { type: RecordType }) {
  ...
  return <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${colors[type]}`}>{RECORD_TYPE_LABELS[type]}</span>;
}
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${colors[status] ?? "bg-zinc-800 text-zinc-300"}`}>{status}</span>;
}
