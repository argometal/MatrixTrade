        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            {evidenceCount !== undefined && (
              <p className="mt-2 text-xs text-teal-500">{evidenceCount} evidencia{evidenceCount !== 1 ? "s" : ""}</p>
            )}
          </div>
...
export function EvidenceCard({ item }: { item: Evidence }) {
  return (
    <Card>
      ...
      <p className="text-xs text-zinc-500">{formatDate(item.date)} · {item.source}</p>
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            {recordCount !== undefined && (
              <p className="mt-2 text-xs text-zinc-500">{recordCount} registro{recordCount !== 1 ? "s" : ""}</p>
            )}
          </div>
          <span className="text-zinc-600">›</span>
        </div>
