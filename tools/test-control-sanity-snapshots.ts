...
// 1. Prompt ID Protocol appears in canonical Mechanics snapshot
{
  ...
  assert.ok(brief.includes("APPLY JSON PASTE"));
  assert.ok(brief.includes("Buy 1 share at $315"));
  assert.ok(brief.includes("Do not send plannedQuantity"));
  assert.ok(brief.includes("Exception — Apply JSON paste"));
}
