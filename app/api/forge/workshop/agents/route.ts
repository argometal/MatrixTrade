...
import { listWorkshopReleaseDownloads } from "@/lib/workshop/agent-exe-paths.server";
...
export async function GET() {
  ...
  const downloads = listWorkshopReleaseDownloads();
  const canLaunchFromServer = process.platform === "win32";
  return NextResponse.json({ ok: true, ...status, downloads, canLaunchFromServer });
}
