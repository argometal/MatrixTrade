...
import { MISTAKE_LABELS } from "@/lib/review";
import type { Trade } from "@/lib/types";

/** Classic light-theme Review page — preserved for reference. */
export function LegacyReviewPage({
  attentionItems,
  unreviewed,
  pendingInbox,
  needsPlaybook,
    ...
    <div className="space-y-8">
      ...
      {pendingInbox.length > 0 && (
        <section className="space-y-3">
          ...
          <Link href="/mta/inbox" className="text-sm text-zinc-600 hover:underline">
            Open full inbox →
          </Link>
        </section>
      )}
          ...
          <ul className="divide-y divide-zinc-100 rounded-lg border border-zinc-200 bg-white shadow-sm">
            {needsPlaybook.map((trade) => (
              <li key={trade.id} className="flex items-center justify-between px-4 py-3 text-sm">
                ...
                <Link href={`/mta/trades/${trade.id}`} className="font-medium underline">
                  Assign →
                </Link>
              </li>
            ))}
            ...
            {reviewedTrades.map((trade) => (
              <li key={trade.id} className="px-4 py-3 text-sm">
                <div className="flex items-center justify-between">
                  <Link href={`/mta/trades/${trade.id}`} className="font-medium hover:underline">
                    {trade.id} · {trade.ticker}
                  </Link>
                  <Link
                    href={`/mta/trades/${trade.id}/review`}
                ...
                {trade.lesson && <p className="mt-1 text-zinc-600">{trade.lesson}</p>}
              </li>
            ))}
          </ul>
        )}
