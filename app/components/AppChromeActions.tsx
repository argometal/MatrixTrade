...
/** MTA chrome: inbox + A mark systems menu (no ···, no second A). */
export function MatrixAppChromeActions({ pendingInboxCount = 0 }: { pendingInboxCount?: number }) {
  return (
    <>
      <Link href="/mxt/inbox" aria-label="Proposals" title="Proposals" className={iconBtn}>
        🔔
        {pendingInboxCount > 0 ? (
...
/** Argus chrome: inbox + A mark systems menu (no ···, no second A). */
