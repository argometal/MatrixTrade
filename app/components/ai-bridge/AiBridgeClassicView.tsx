...
export function AiBridgeClassicView({
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Classic view — Control → Apply, or paste here → Proposals → Apply.
          </p>
        </div>
        {viewToggle}
      </header>
      ...
      <SystemSection id="inbox" title="Proposals" description="Pending proposals — Apply to write Supabase.">
          ...
          <ul className="mt-4 divide-y divide-zinc-100 rounded-lg border border-zinc-200 text-sm">
            {pendingInboxPreview.map((item) => (
              <li key={`${item.origin}-${item.id}`} className="flex justify-between gap-3 px-4 py-2">
                ...
                <Link href={`/mxt/inbox/${item.id}?origin=${item.origin}`} className="underline">
                  Review
                </Link>
              </li>
            ))}
      ...
      <nav className="flex gap-4 text-sm">
        <Link href="/mxt/system" className="text-zinc-600 hover:underline">
          System (sync) →
        </Link>
