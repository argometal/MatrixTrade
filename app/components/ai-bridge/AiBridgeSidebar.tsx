...
const BRIDGE_RULES = [
  ...
  "Supabase (or local JSON) is the source of truth.",
] as const;
...
export function AiBridgeSidebar({
  overview,
  pendingInboxCount,
}: {
  ...
  return (
    <aside className="space-y-5 lg:sticky lg:top-6 lg:self-start">
      <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
        <h2 className="text-sm font-semibold text-zinc-900">How AI Bridge works</h2>
        <ol className="mt-3 space-y-2 text-xs text-zinc-600">
          <li>1. Copy Snapshot</li>
          <li>2. AI analyzes + responds</li>
          <li>3. Paste AI Block here</li>
      ...
      <section className="rounded-2xl border border-violet-100 bg-violet-50/60 p-4">
        <h2 className="text-sm font-semibold text-violet-950">AI Bridge rules</h2>
        <ul className="mt-3 space-y-2">
          {BRIDGE_RULES.map((rule) => (
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-1 text-xs text-zinc-500">Transparency only — use Load sample format in panel 3.</p>
      ...
      <nav className="flex flex-wrap gap-3 text-xs text-zinc-500">
        ...
        <Link href="/system" className="hover:text-zinc-800 hover:underline">
          System
        </Link>
        <Link href="/playbook" className="hover:text-zinc-800 hover:underline">
          Playbooks
        </Link>
      </nav>
    </aside>
