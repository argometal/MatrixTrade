...
export function EntityCreateForm({
  ...
  return (
    <form action={createEntityAction} className="mb-6 space-y-3 rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
      ...
      {error === "name" && <p className="text-sm text-amber-400">VALIDATION: Name is required.</p>}
      {error === "kind" && <p className="text-sm text-amber-400">VALIDATION: Invalid type.</p>}
      {errorLayer && errorMsg ? (
        <p className="text-sm text-amber-400">
          {errorLayer.toUpperCase()}: {errorMsg}
        </p>
      ) : null}
