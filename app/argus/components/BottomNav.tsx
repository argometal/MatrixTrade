...
const links = [
  { href: "/argus", label: "Home", icon: "⌂" },
  { href: "/argus/entries", label: "Entries", icon: "▤" },
  { href: "/argus/contacts", label: "Contacts", icon: "◎" },
];
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-zinc-800 bg-zinc-950/95 backdrop-blur-lg">
      <div className="mx-auto flex max-w-lg items-stretch justify-around px-2 py-2">
        {links.map((link) => {
          const active =
            link.href === "/argus" ? pathname === "/argus" : pathname.startsWith(link.href);
          return (
            <Link
              key={link.href}
              href={link.href}
