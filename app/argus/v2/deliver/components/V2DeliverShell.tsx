  ...
  return (
    <div className="deliver-export-shell mx-auto max-w-6xl px-4 py-6 lg:px-8">
      ...
      <section className="mb-8">
        <h2 className="mb-4 text-base font-semibold text-zinc-100">Choose package type</h2>
            ...
            return (
              ...
              >
                <div className="mb-2 flex items-center justify-between gap-2">
                  <span className="text-base font-semibold text-zinc-100">{pkg.title}</span>
                  {selected ? <span className="text-violet-400">✓</span> : null}
                </div>
                <p className="mb-3 text-sm leading-relaxed text-zinc-400">{pkg.description}</p>
                <span className={`inline-flex rounded-full px-2 py-0.5 text-sm font-medium ring-1 ${BADGE_TONE[pkg.badgeTone]}`}>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <V2Card className="p-5">
          <h2 className="mb-4 text-base font-semibold text-zinc-100">Define scope</h2>
          ...
          <div className="mb-5 space-y-2">
            <p className="text-sm font-medium text-zinc-400">Include</p>
            {[
          ...
          {!privateConfigured ? (
            <p className="mt-2 text-sm text-zinc-400">Set ARGUS_PRIVATE_PIN to enable private export.</p>
          ) : includePrivate && !privateUnlocked ? (
            <p className="mt-2 text-sm text-amber-400/90">Unlock protected records from the top bar before generating.</p>
          ) : null}
        </V2Card>

        <V2Card className="flex h-fit flex-col p-5">
          <div className="mb-4 flex items-center justify-between gap-2">
            <h2 className="text-sm font-bold uppercase tracking-wider text-zinc-400">Package summary</h2>
            <button
              type="button"
          ...
          {preview?.containsPrivate && !includePrivate ? (
            <p className="mt-3 text-sm text-amber-400/90">Scope contains private records not included unless toggled on.</p>
          ) : null}
          {!packageAvailable ? (
            <p className="mt-3 text-sm text-zinc-400">This package type is not available yet. Use Evidence Vault.</p>
          ) : null}
        </V2Card>
      </div>
