        ...
        <div className="border-b border-zinc-800/80 px-4 py-4 lg:px-5">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-sm text-zinc-400">Knowledge areas and themes</p>
            </div>
          ...
          {tagChips.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">
              {tagChips.map((chip) => (
                <span
                  key={chip.name}
                  className="rounded-full bg-zinc-800/80 px-2 py-0.5 text-sm text-zinc-400"
                >
        ...
        <div className="flex-1 overflow-auto">
          {filtered.length === 0 ? (
            <div className="px-5 py-16 text-center">
              ...
              <p className="mt-1 text-sm text-zinc-400">Create a topic and link it to projects, orgs, people, or events.</p>
              <div className="mt-4">
                <V2CreateEntityButton
                  kind="topic"
                  label="+ Topic"
            ...
            <table className="w-full text-left text-sm">
              <thead className="sticky top-0 bg-zinc-950/95 text-sm uppercase tracking-wide text-zinc-400">
              ...
              <tbody>
                {filtered.map((row) => (
                  ...
                  >
                    ...
                    <td className="hidden px-2 py-3 text-zinc-400 sm:table-cell">{row.category}</td>
                    <td className="hidden px-2 py-3 text-sm text-zinc-400 md:table-cell">
                    ...
                    <td className="px-2 py-3 text-zinc-400">{row.lastActivity}</td>
                    <td className="px-4 py-3 lg:px-5">
                      <span className="text-zinc-400">★</span>{" "}
                      <span className="tabular-nums text-zinc-400">{row.entryCount}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <section className="min-w-0 flex-1 overflow-y-auto bg-zinc-950/50">
        {selected ? (
          <div className="flex h-full flex-col p-5">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-sm text-amber-300 ring-1 ring-amber-500/25">
                    {selected.category}
                  </span>
                  <span className="text-zinc-400">★</span>
                </div>
            ...
            <h3 className="mb-3 text-base font-semibold text-zinc-100">Recent entries</h3>
            {selected.recentEntries.length === 0 ? (
              <p className="text-sm text-zinc-400">No journal entries linked yet.</p>
            ) : (
                  ...
                  <li key={entry.id}>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-base font-medium text-zinc-200">{entry.title}</span>
                        <span className="text-sm text-violet-400">{entry.kind}</span>
                      </div>
                      <p className="mt-0.5 text-sm text-zinc-400">{entry.meta}</p>
                    </Link>
                  </li>
                ))}
              </ul>
