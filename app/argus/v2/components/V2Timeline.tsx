    ...
    <div>
      ...
      <div className="space-y-8">
        {groups.map(([date, items]) => (
          <div key={date}>
            <p className="mb-4 text-sm font-semibold tracking-wider text-zinc-400">{formatV2Date(date)}</p>
              ...
              {items.map((entry) => (
                <article key={entry.id} className="relative">
                  ...
                  >
                    ...
                    {entry.body ? (
                      <p className={`mt-1.5 text-zinc-400 ${compact ? "line-clamp-2 text-sm" : "text-sm leading-relaxed"}`}>
                        {entry.body}
                      </p>
                    ) : null}
                    {entry.author ? <p className="mt-2 text-sm text-zinc-400">{entry.author}</p> : null}
                  </div>
                </article>
              ))}
            </div>
          ...
          <div className="relative space-y-3 border-l border-zinc-800 pl-4">
            {items.map((entry) => (
              <div key={entry.id} className="relative">
                ...
                <p className="text-sm font-medium text-zinc-300 line-clamp-1">{entry.title}</p>
                <p className="text-sm text-zinc-400">{entry.time ?? "—"}</p>
              </div>
            ))}
          </div>
        </div>
